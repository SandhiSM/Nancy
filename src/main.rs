...
fn main() {
    ...
    let recalculate = || {
        ...
        .calculate();
        if instance.number == confirmation {
            return true;
        } else {
            return false;
        }
...
fn construct() -> Number {
    ...
    let alphabet_to_number = |arguments: &Vec<String>| {
        ...
        'first: for character in arguments[2].chars() {
    ...
    let minus_checker = move |number: Vec<String>| {
...
impl Number {
    fn calculate(self) -> Vec<String> {
        let convert_to_dec_int = || {
            ...
            for number in numbers {
                converted_number = (self.from as u128).pow(length - 1) * number;
                let new_number = converted_number + old_number;
        ...
        let convert_to_dec_float = || {
            ...
            let mut converted_number = 0.0;
            let mut old_number = 0.0;
            for number in numbers {
                converted_number = (self.from as f64).powi(length - 1) * number;
                let new_number = converted_number + old_number;
                old_number = new_number;
                length -= 1;
            }
        ...
        let number_to_alphabet = move |number: u128| {
        ...
        let float_calculate = |number: f64| {
            ...
            if output[output.len()] == ".".to_string() {
                output.pop();
            } else {
                ()
            }
